...
func (h *HandlerV1) Graphql(c *gin.Context) {
	...
	// Create a new request to Hasura
...
func GenerateREST() {
	...
	for _, table := range tables {
		if table == "jupiter" {
			...
			payload := generateBulkPayload(table, operations, pkFields)
...
func applyBulkOperation(payload BulkOperation) error {
	body, err := json.Marshal(payload)
	if err != nil {
		return err
	}
